...
const modTareas = {
    ...
    view(id) {
        ...
        const footer = `
            ...
            ${(auth.esAdmin() || auth.esTecnico()) ? `<button class="btn btn-danger" onclick="ui.closeModal(); modTareas.delete('${id}')">Eliminar</button>` : ''}
        `;
    ...
    async delete(id) {
        ui.confirm('Eliminar Tarea', '¿Estás seguro de eliminar esta tarea?', async () => {
            try {
                ...
                ui.showToast('Error al eliminar tarea: ' + e.message, 'error');
            }
        });
    }
};
